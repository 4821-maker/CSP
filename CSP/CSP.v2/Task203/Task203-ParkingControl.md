...
The company **EstaciONE**, which owns a large parking lot, faces challenges in efficiently managing its parking spots, which are divided among **daily users, monthly subscribers, and rotating customers**. Each spot is exclusively reserved for a specific category, so a customer can only park if there is an available spot in their corresponding category.
...
You must upload the following to university Moodle by **11:59 PM on 06/06/2022**, in the designated space:
