...
Several students from the **Institute of Computing** are preparing to participate in the **Brazilian Programming Marathon (SBC)** ([http://maratona.sbc.org.br](http://maratona.sbc.org.br)). In this competition, teams submit source code to solve problems, using evaluation systems known as **Online Judges (OJ)**.
...
The team must submit a **report in SBC article format** containing all analyses and results. Submission is due via **University Moodle** by **11:59 PM on 27/06/2022**. The report must include:
